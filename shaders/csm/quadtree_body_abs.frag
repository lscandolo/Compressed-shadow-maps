							   ...
							   const uint8_t lodlevel)
{
	...
	while(true) 
	{
		...
		if (ceilLevel) {
			ceilVal = getQuadtreeValue(search.valueptr);
		} else if (floorLevel) {
			float floorVal = isempty ? getQuadtreeValue(search.valueptr) : ceilVal;
		...
		{
			// Return value from the last stored position
			float floorVal = getQuadtreeValue(search.valueptr);
		...
		{
			float floorVal = getQuadtreeValue(search.nodeptr);
...
float evaluateQuadtreeHierarchicalPCF(vec4 coord,
									  const float bias, const uint8_t pcf, 
									  const float lodlevel, const float pixelsize)
{
	...
	if (hasOnlyOneValue)
	{
		float lightDepth = getQuadtreeValue(search.valueptr);
			  		...
			  		const uint8_t lodlevel)
{
	...
	while (true)
	{
		...
		{
			return getQuadtreeValue(search.valueptr);
		}
