...
void ImGuiFileDialog::ScanDir(std::string vPath)
{
	...
	if (n >= 0)
	{
		for (i = 0; i < n; i++)
		{
			...
			if (infos.fileName != ".") m_FileList.push_back(infos);
		}
...
bool ImGuiFileDialog::FileDialog(const char* vName, const char* vFilters, std::string vPath, std::string vDefaultFileName)
{
	...
	ImGui::SetNextWindowSize(ImVec2(800.f, 550.f), ImGuiCond_Once);
	ImGui::Begin(vName);
	...
	for (std::vector<FileInfoStruct>::iterator it = m_FileList.begin(); it != m_FileList.end(); ++it)
	{
		...
		if (infos.type == 'f' && m_CurrentFilterExt.size() > 0 && m_CurrentFilterExt != "*" && infos.ext != m_CurrentFilterExt)
		{
			show = false;
		}
		if (show == true)
		{
			...
			if (infos.type == 'f' && ImGui::IsItemHovered() && ImGui::IsMouseDoubleClicked(0))
			{
				IsOk = true;
				res = true;
			}
