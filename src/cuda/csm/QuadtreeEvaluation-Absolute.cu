...
///////////////////////////////////////////////////////////
			  		    ...
			  		    const uint8_t lodlevel)
	{
		...
		while (true)
		{
			...
			{
				return getQuadtreeValue(qdata, search.valueptr);
			}
			...
			{
				search.valueptr = search.nodeptr;
				return getQuadtreeValue(qdata, search.valueptr);
			}

			// If the node is not empty, store the position of the value
			if (is_innernode(search.flags))
				search.valueptr = search.nodeptr + 1;
			
		}

		return -1.f;	// Should never be reached
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
									 ...
									 const uint8_t lodlevel)
	{
		...
		while(true) 
		{
			...
			if (ceilLevel) {
				ceilVal = getQuadtreeValue(qdata, search.valueptr);
			} else if (floorLevel) {
				float floorVal = isempty ? getQuadtreeValue(qdata, search.valueptr) : ceilVal;
			...
			if (is_nonexistantnode(childflags))
			{
				...
				float floorVal = getQuadtreeValue(qdata, search.valueptr);
			...
			{
				float floorVal = getQuadtreeValue(qdata, search.nodeptr);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
							 ...
							 const level_t lastCommonLevel)
	{
		...
		while(search.level < lastCommonLevel) 
		{
			...
			// Try to traverse the next child (if it exists)
			uint8_t childflags = (flags >> (nextchild << 1)) & 3;
			if (is_nonexistantnode(childflags))
			{
				// Return value from the last stored position
				return true;
			...
			bool isempty = is_emptynode(childflags);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
										  ...
										  float lodlevel, const float pixelsize)
	{
		...
		if (hasOnlyOneValue)
		{
			float lightDepth = getQuadtreeValue(qdata, search.valueptr);
		...
		for (int8_t x = -pcf; x <= pcf; ++x) {
			for (int8_t y = -pcf; y <= pcf; ++y) {
				...
				int2 iNextcoords =  iQueryCoords +  make_int2(x, y) * ksize;
...
////////////////////////////////////////////////////////////////////////////////////////////////
...
#if 1
		float lightDepth = getDepthValue(qdata, queryCoords, lodlevel);

		if (pointDepth >= lightDepth + bias)
			shadow = 1.f;

#else
		float fractLodLevel = lodlevel - floor(lodlevel);
		float2 lightDepth = getDepthValueHierarchical(qdata, search, queryCoords, int(lodlevel));
		if (pointDepth >= lightDepth.x + bias)
			shadow += fractLodLevel;
		if (pointDepth >= lightDepth.y + bias)
			shadow += 1.f - fractLodLevel;
#endif
						 ...
						 const cudaSurfaceObject_t resultSurf)
	{
		...
		if (val)
			surf2Dwrite(val, resultSurf, x * sizeof(uint8_t), y);
	}

	void evaluateShadow_Absolute(const size_t width,
		                         const size_t height,
